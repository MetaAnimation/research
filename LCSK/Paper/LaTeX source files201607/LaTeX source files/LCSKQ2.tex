...
%\input{INTRODUCTION}
%introduction
With the development of information technologies such as GIS, as well as the increasing popularity of services such as Google Earth and Baidu Lvyou, large volumes of geo-textual objects (e.g., facilities, attractions) are available in spatial databases. The geo-textual datasets enable to study the spatial keyword queries \cite{cong2012efficient, de2008keyword, wu2012joint, zhang2009keyword} for answering queries with semantic constraints.

Previous works on spatial keyword queries can be roughly classified into two categories based on the answer granularity: (1) some proposals find individual objects. Typically, given a location and a set of keywords as arguments, this type of query \cite{cong2009efficient,  cong2012efficient, de2008keyword, shang2012user} retrieves individual objects that each cover all query keywords, and (2) others ask for a group of objects. In a wide spectrum of applications, whereas multiple objects are required to cover query keywords collectively. Toward this goal, mCK \cite{zhang2009keyword, zhang2010locating}, CoSKQ \cite{cao2011collective, long2013collective}, BKC \cite{deng2015best} and SGK \cite{cao2015efficient} are investigated recently. To the best of our knowledge, few studies concern the \textit{keyword level} (e.g., the level of attractions, hotels or personal skills), which is increasingly important for users to make decisions in real applications.

In this work, we explore a novel query paradigm called LCSK. We enhance CoSKQ from the following three aspects. First, we consider the \textit{level vector} of objects in the underlying database $O$. We denote by $o.\omega$ the associated keywords with an object $o \in O$. Specifically, the level vector of $o \in O$ is an integer vector, denoted by $o.\nu$. The $i$th element of a \textit{level vector}, i.e., $o.\nu^i$, represents the level of $i$th keyword in $o.\omega$, i.e., $o.\omega^i$. Second, we introduce the \textit{weight vector} into the query definition for capturing the user-specified weights assigned to different levels. Third, we consider the cost distance in \cite{chen2014efficient} as the cost measurement. The LCSK query has numerous real applications such as resource scheduling, emergency rescue. Next, we present an example of the emergency rescue task.
...
%\input{RELATED_WORK}
%RELATED WORK
In this section, we review works related to the LCSK query, mainly focusing on the conventional spatial keyword queries and the collective spatial keyword queries.

\subsection{Conventional Spatial Keyword Queries}
The conventional spatial keyword queries \cite{cong2009efficient, de2008keyword} take a location and a set of keywords as arguments, and ask for the objects that cover all the query keywords. There are lots of efforts on the conventional spatial keyword queries. By combining with other spatial queries, several variations of the conventional spatial keyword queries are studied in spatial databases. We review them as follows.
...
\section{Proposed Exact Algorithm}
